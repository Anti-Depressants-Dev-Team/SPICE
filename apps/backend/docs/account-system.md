...
```bash
pnpm --filter @spice/backend db:migrate
```

For local database prototyping without migration files, `pnpm --filter @spice/backend db:push` can also apply the schema.
