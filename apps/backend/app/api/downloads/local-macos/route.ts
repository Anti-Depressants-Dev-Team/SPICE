...
import { localMacosDownloadUrl } from '@/lib/local-updates';
...
export function GET() {
  ...
  const downloadUrl = localMacosDownloadUrl();
  if (!downloadUrl) {
    return publicJsonResponse({
      error: 'download_unavailable',
      message: 'The SPICE local macOS ZIP has not been published yet.',
    }, { status: 503 });
  }
